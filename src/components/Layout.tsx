...
export const Layout = ({ children }: Props) => {
  ...
  const onMessageReceived = (
    msg: IInboxMessagePreview | ICourierEventMessage | undefined
  ) => {
    switch ((msg as any).data.notificationType) {
      case NOTIFICATION_TYPE_CONTRIBUTION_VERIFIED: {
        const { contributionId, dataValidationStatus, filePath } = (msg as any)
          .data;
        console.log(
          `notificaiton received:${NOTIFICATION_TYPE_CONTRIBUTION_VERIFIED}`
        );
        dispatchEvent(
          new CustomEvent(NOTIFICATION_TYPE_CONTRIBUTION_VERIFIED, {
            bubbles: true,
        ...
        notifyVerified(
          ...
          `${filePath} in Contribution ${contributionId}`,
          RAG_REWARDS.REWARD_FOR_CONTRIBUTORS
        );
        break;
      }
      case NOTIFICATION_TYPE_MODEL_VERIFIED: {
        ...
        console.log(
          `notificaiton received:${NOTIFICATION_TYPE_MODEL_VERIFIED}`
        );
        dispatchEvent(
          new CustomEvent(NOTIFICATION_TYPE_MODEL_VERIFIED, {
            bubbles: true,
            detail: (msg as any).data,
          })
        );
        notifyVerified(
          dataValidationStatus,
          `Model ${modelName}`,
          RAG_REWARDS.REWARD_FOR_MODEL_CREATION
        );
        break;
      }
      case NOTIFICATION_TYPE_REWARD_UPDATED: {
        ...
        dispatchEvent(
          new CustomEvent(NOTIFICATION_TYPE_REWARD_UPDATED, {
            bubbles: true,
            detail: (msg as any).data,
          })
        );
  ...
  return (
    <Box background="#F8FAFB" fill>
      ...
      >
        <div
          className={`w-5/6 flex flex-row items-center md:justify-between gap-x-4`}
        >
          {size === "small" && (
          ...
          <div
            className={`flex ${size === "small" && "w-full"} justify-center`}
          >
            <Box width={{ max: "small" }}>
              <Image
