...
const REWARD_TITLE_MAP=[
  ...
  {key:'DiscordJoinGetRole',title:'Join our Discord and acquire a Flockie Role'},
...
const TaskCenter = ({}:TaskCenterProps) => {
  ...
  const completedTasks=REWARD_TITLE_MAP.filter(({key})=>!rewardsAllowedMap[key])
  const notCompletedTasks=REWARD_TITLE_MAP.filter(({key})=>rewardsAllowedMap[key])
  const firstUncompletedTaskKey = REWARD_TITLE_MAP.find(({ key }) => rewardsAllowedMap[key])?.key;
  ...
  const [twitterFollowLoading, setTwitterFollowLoading] = useState<boolean>(false);
  const [twitterShareLoading, setTwitterShareLoading] = useState<boolean>(false);
  ...
  const getRewardsData=async ()=>{
    ...
    try{
      ...
      const rewardsAllowedResponse=await fetch(`/api/rag/getRewardsAllowedMap?userId=${address?.toLowerCase()}`)
  ...
  useEffect(() => {
    if (twitterAccessToken.accessToken && twitterAccessToken.expiresAt && !twitterAuthHandledRef.current) {
  ...
  const handleTwitterFollow = async () => {
    setLoading(true);
    window.open(
      process.env.NEXT_PUBLIC_TWITTER_FOLLOW_LINK,
      '_blank'
    )
    setLoading(false);
  };
                ...
                <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 items-center justify-center md:justify-end space-x-0 md:space-x-2">
                  ...
                  <div style={{ width: '180px' }}>
                    ...
                    {key === 'DiscordConnect' && discordName && (
                      <PrimaryButton size="small" label={discordName} style={{ width: '180px' }} />
                    )}
                  </div>
                </div>
              </div>
