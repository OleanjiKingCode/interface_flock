...
const Leaderboard = ({ modelId }: LeaderboardProps) => {
  ...
  const getLeaderboard = async () => {
    ...
    try {
      const response = await fetch(
        `/api/rag/getLeaderboard?modelId=` + (modelId || "")
      );
      const result = await response.json();
      setLeaderboards(result);
      if (address) {
        const rewards = result.find(
          (item: any) =>
            item.user.wallet.toLowerCase() === address?.toLowerCase()
        );
  ...
  return modelId ? (
    <div className="relative overflow-x-auto">
      <table
        className="w-full text-sm text-left rtl:text-right border-separate bg-white"
        style={{
      ...
      >
        {!modelId && (
          <caption
            className="text-2xl font-extrabold text-black text-left bg-white"
            style={{
            ...
            <>
              {reward ? (
                <tr className="bg-blue-100">
                  <th
                    scope="row"
                    className="px-6 py-4 font-medium whitespace-nowrap rounded-l-2xl"
                  >
                    {rank}
                  </th>
                ...
                <tr className="bg-blue-100">
                  ...
                  <td className="px-6 py-4 rounded-r-2xl text-blue-600">0</td>
                </tr>
              )}
            </>
          )}
